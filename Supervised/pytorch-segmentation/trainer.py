...
for epoch in range(epoches):
    running_loss = 0.0
    # for i, (images, labels_group) in enumerate(trainloader , 0):
    for i, (images, labels_group) in tqdm(enumerate(trainloader), total= len(trainloader)):
        print(i)
        if torch.cuda.is_available():
...
# torch.save(model.state_dict(), "fcn-deconv.pth")
